package SqlBatch::CommitInstruction;

# ABSTRACT: Base class for an SQL transaction-commit instruction
...
sub run {
    ...
    if ($force_autocommit) {
	$self->show_warning("Explicit commit of a transaction is not enforced due to 'force_autocommit' in configuration");
    } else {
	eval {
	    my $rv = $self->databasehandle->commit;
